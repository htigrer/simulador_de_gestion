...
function mostrarPregunta(){
  ...
  document.getElementById('btnNext').onclick = () => { 
    // Si no ha respondido aún, verificar automáticamente antes de avanzar
    if (respuestas[idx] == null) {
      verificarRespuestaAutomatica(tipo);
    }
    if (idx<ronda.length-1) { idx++; mostrarPregunta(); } else { finalizar(false); } 
  };
  document.getElementById('btnFin').onclick  = () => {
    // Verificar la pregunta actual antes de finalizar
    if (respuestas[idx] == null) {
      verificarRespuestaAutomatica(tipo);
    }
    finalizar(false);
...
function verificarRespuestaAutomatica(tipo){
  const q = ronda[idx];
  const wrap = document.getElementById('opciones');
  ...
  switch(tipo){
    ...
    case 'emparejamiento':
      const pares = q.pares;
      const derechas = respuestas[idx]?.derechas || [];
      const emparejamientos = [];
      wrap.querySelectorAll('select').forEach((sel, i) => {
        const valorSeleccionado = sel.value;
        if(valorSeleccionado !== ""){
      ...
      // Guardar derechas para verificación posterior
      const derechasActuales = Array.from(wrap.querySelectorAll('select option:not([value=""])')).slice(0, pares.length).map(opt => opt.textContent);
      respuestas[idx] = { emparejamientos, derechas: derechasActuales };
      break;
...
function verificarRespuesta(tipo){
  ...
  switch(tipo){
    ...
    case 'multiple_seleccion':
      ...
      const correctasArr = q.respuestas_correctas || [];
      
      // CASO ESPECIAL: Si seleccionó TODAS las opciones y todas son correctas
      const todasLasOpciones = q.opciones.length;
      const seleccionoTodas = seleccionadas.length === todasLasOpciones;
      const todasSonCorrectas = correctasArr.length === todasLasOpciones;
      
      if(seleccionoTodas && todasSonCorrectas){
        esCorrecta = true;
      } else {
        // Verificar que tenga la misma cantidad Y que todas las seleccionadas sean correctas
        // Y que todas las correctas estén seleccionadas
        esCorrecta = seleccionadas.length === correctasArr.length && 
                     seleccionadas.every(s => correctasArr.includes(s)) &&
                     correctasArr.every(c => seleccionadas.includes(c));
      }
      break;
...
function mostrarFeedback(ok, q, tipo){
  ...
  if(ok){
    ...
    if(tipo === 'multiple'){
      ...
      const correctasTexto = q.respuestas_correctas.map(i => `${String.fromCharCode(65 + i)}) ${q.opciones[i]}`).join('<br>');
      const seleccionadas = respuestas[idx] || [];
      const seleccionadasTexto = seleccionadas.map(i => `${String.fromCharCode(65 + i)}) ${q.opciones[i]}`).join('<br>');
      
      detalleIncorrecto = `
        <div class="mt-2">
          <p class="font-semibold mb-1">Tu selección:</p>
          <div class="ml-4 text-sm">${seleccionadasTexto || '<em>Ninguna seleccionada</em>'}</div>
          <p class="font-semibold mt-3 mb-1">Respuestas correctas:</p>
          <div class="ml-4 text-sm">${correctasTexto}</div>
        </div>
      `;
    }
...
function deshabilitarOpciones(tipo, esCorrecta){
  ...
  if(tipo === 'multiple'){
    ...
    document.querySelectorAll('#opciones input[type="checkbox"]').forEach((cb, i) => {
      ...
      const esCorrecta = q.respuestas_correctas.includes(i);
      const fueSeleccionada = seleccionadas.includes(i);
      
      // Si es correcta, mostrar en verde
      if(esCorrecta){
        label.classList.remove('border-gray-200', 'bg-white');
        label.classList.add('border-green-500', 'bg-green-50', 'ring-2', 'ring-green-300');
      }
      // Si fue seleccionada pero NO es correcta, mostrar en rojo
